...
public partial class MeasurementViewModel : ObservableObject
{
    ...
    private void GoToStep(object? stepParameter)
    {
        ...
        switch (stepIndex)
        {
            ...
            case 4 when !CanGoToStep4:
                _logHelper?.Warning("无法进入步骤4：需要至少一个视频");
                return;
        }

        CurrentStep = stepIndex;

        // 进入 Step4 时同步上下文到子 VM
        if (stepIndex == 4)
        {
    ...
    private bool CanSelectVideo() => HasMeasurementRecord;
    ...
    private void StartRecording()
    {
        ...
        _logHelper?.Information($"开始录制，时长: {RecordDurationSeconds}s, 模式: {SelectedCaptureLayout}");
    }
