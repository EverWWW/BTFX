    ...
    private void ComposeGaitParameters(IContainer container, Report report)
    {
        ...
        container.Column(column =>
        {
            ...
            column.Item().Table(table =>
            {
                ...
                table.Cell().Element(c => CreateParameterCell(
                    c,
                    "步频",
                    GaitCadenceCalculator.PreferCycleDerived(gait?.GaitCycleDurationS, gait?.Cadence),
                    "step/min"));
                ...
                table.Cell().Element(c => CreateParameterCell(c, "双支撑时间", gait?.DoubleSupportTimeS, "s"));
