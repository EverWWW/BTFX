...
def build_transcode_cmd(avi_file: Path, mp4_file: Path) -> list[str]:
    """
    ...
    return [
        ...
        "-vf", "scale=in_range=pc:out_range=tv,format=yuv420p",    # 像素范围与格式转换，提升 MP4 兼容性
...
def run_transcode(avi_file: Path, mp4_file: Path, camera_name: str) -> None:
    """
    执行单台相机的 AVI -> MP4 转码。
    """
    cmd = build_transcode_cmd(avi_file, mp4_file)
