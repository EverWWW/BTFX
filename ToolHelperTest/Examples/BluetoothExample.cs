...
public class BluetoothExample
{
    ...
    public static async Task ScanDevicesAsync()
    {
        ...
        try
        {
            ...
            bluetooth.ScanCompleted += (sender, e) =>
            {
                ...
                Console.WriteLine($"扫描耗时: {e.Duration.TotalSeconds:F1} 秒");
            };
    ...
    public static async Task SendReceiveDataAsync()
    {
        ...
        try
        {
            ...
            if (await bluetooth.ConnectAsync())
            {
                ...
                // 启动接收
                _ = bluetooth.StartReceivingAsync();
    ...
    /// <summary>
    /// 示例 4: 手动创建 BluetoothHelper（不使用依赖注入）
    ...
    public static async Task ManualCreationAsync()
    {
        ...
        using var bluetooth = new BluetoothHelper(Options.Create(options), logger);
        ...
        bluetooth.ErrorOccurred += (s, e) =>
            Console.WriteLine($"错误: [{e.ErrorType}] {e.Message}");
    ...
    public static async Task BleDeviceAsync()
    {
        ...
        services.AddBluetooth(options =>
        {
            options.ScanBleOnly = true;
            options.DeviceAddress = "00:11:22:33:44:55";
            // 通用属性服务UUID（示例）
            options.ServiceUuid = "0000180f-0000-1000-8000-00805f9b34fb";  // Battery Service
            // 电池电量特征UUID（示例）
            options.CharacteristicUuid = "00002a19-0000-1000-8000-00805f9b34fb";  // Battery Level
        });
    ...
    /// 演示如何配对蓝牙设备
    /// </summary>
    public static async Task PairDeviceAsync()
    {
        Console.WriteLine("=== 蓝牙设备配对示例 ===\n");

        using var bluetooth = new BluetoothHelper();
        ...
        // 获取第一个未配对的设备
        var unpairedDevice = devices.FirstOrDefault(d => !d.IsPaired);
        if (unpairedDevice == null)
        {
            Console.WriteLine("所有设备都已配对");
            return;
        }

        Console.WriteLine($"\n尝试配对设备: {unpairedDevice.Name} ({unpairedDevice.Address})");

        // 配对设备
        var success = await bluetooth.PairDeviceAsync(unpairedDevice);
        Console.WriteLine($"配对结果: {(success ? "成功" : "失败")}");
    }
    ...
    public static async Task RunAllExamplesAsync()
    {
        ...
        Console.WriteLine("  4. ManualCreationAsync  - 手动创建BluetoothHelper");
        Console.WriteLine("  5. BleDeviceAsync       - BLE低功耗蓝牙模式");
        Console.WriteLine("  6. PairDeviceAsync      - 设备配对");
        Console.WriteLine();
        Console.WriteLine("请输入示例编号 (1-6) 或 'a' 运行全部, 'q' 退出：");
            ...
            try
            {
                switch (input)
                {
                    ...
                    case "q":
                        ...
                        return;
                    default:
                        Console.WriteLine("无效输入，请输入 1-6, 'a' 或 'q'");
                        continue;
                }
